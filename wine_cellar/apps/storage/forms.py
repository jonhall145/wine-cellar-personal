...
class StorageForm(forms.Form):
    ...
    name = forms.CharField(
        max_length=100,
        help_text="Enter the name of the storage.",
    )
    description = forms.CharField(
        required=False,
        help_text="Enter a description of the storage.",
    )
    location = forms.CharField(
        max_length=100,
        help_text="Enter the location of the storage.",
    )
    ...
    is_cold = forms.BooleanField(
        required=False,
        help_text="Check if this is a cold storage location (refrigerated).",
    )
    is_default = forms.BooleanField(
        required=False,
        help_text="Check to make this the default storage for new bottles.",
    )
    ...
    def clean_cell_mask(self):
        ...
        try:
            ...
            raise forms.ValidationError("Invalid cell mask data.")
        if not isinstance(mask, list):
            raise forms.ValidationError("Cell mask must be a list.")
        for item in mask:
            if (
                ...
                raise forms.ValidationError(
                    "Each cell in mask must be a [row, column] pair."
                )
        ...
        if rows < 1 or columns < 1:
            raise forms.ValidationError(
                "Rows and columns must be at least 1 when providing a cell mask."
            )

        for row, column in mask:
            # Coordinates are 1-based and must be within [1, rows] and [1, columns].
...
class StockAddForm(forms.Form):
    ...
    rating = forms.IntegerField(
        ...
        help_text="Rate this bottle from 0 to 3 stars.",
    )
    ...
    def clean_row(self):
        ...
        if storage and row:
            if storage.rows == 0:
                raise forms.ValidationError(
                    "The selected storage has no rows.", code="redundant_row"
                )
            if row > storage.rows:
                raise forms.ValidationError(
                    "The selected row exceeds the number of rows in the storage.",
                    code="row_exceeds",
                )
        return row
    ...
    def clean_column(self):
        ...
        if storage and column:
            if storage.columns == 0:
                raise forms.ValidationError(
                    "The selected storage has no columns.", code="redundant_column"
                )
...
class StorageItemEditForm(forms.Form):
    ...
    storage = forms.ModelChoiceField(
        queryset=Storage.objects.none(),
        help_text="Select the storage location.",
    )
    row = forms.IntegerField(
        required=False,
        min_value=0,
