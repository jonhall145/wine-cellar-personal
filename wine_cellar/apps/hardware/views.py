...
def sync_operations(request):
    ...
    try:
        ...
        for op in operations:
            ...
            if timestamp_str:
                try:
                    ts_normalized = timestamp_str.replace("Z", "+00:00")
...
class RackConfigView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        threshold = int(request.POST.get("auto_apply_threshold", 90))
        config.auto_apply_threshold = threshold
        config.enable_hourly_snapshots = "enable_hourly_snapshots" in request.POST
        config.enable_daily_reconciliation = (
            "enable_daily_reconciliation" in request.POST
        )
