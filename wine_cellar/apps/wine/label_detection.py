...
def auto_canny(
    image: "NDArray[np.uint8]", sigma: float = 0.33
) -> "NDArray[np.uint8]":
...
def detect_label_contour(
    ...
    # 1. Preprocessing: Convert to Gray and apply CLAHE
    ...
    #    This highlights the "structure" of the label (text blocks) better
    #    than simple edges.
    ...
    #    This connects gaps. If there is glare cutting through the label,
    #    this fills it in. We use a wide rectangular kernel because text
    ...
    for c in contours:
        ...
        #    This finds the bounding box of the blob, even if the edges
        #    are jagged/curved.
        rect = cv2.minAreaRect(c)
        box = cv2.boxPoints(rect)
        box = box.astype(np.int32)

        # Calculate aspect ratio of the rect to ensure it's "label-shaped"
        # (usually taller than wide)
        # Note: minAreaRect width/height order depends on angle, so we just
...
def detect_and_crop_label(
    pil_image: Image.Image, target_height: int = 225
