...
class TestHardwareAuth:
    ...
    def test_invalid_token(self, client):
        ...
        url = reverse("hardware-api:sync")
        response = client.post(url, HTTP_AUTHORIZATION="Token invalid-token")
        assert response.status_code == HTTPStatus.UNAUTHORIZED
    ...
    def test_valid_token(self, client, hardware_device_factory):
        ...
        url = reverse("hardware-api:sync")
        response = client.post(url, HTTP_AUTHORIZATION=f"Token {device.api_token}")
        # Empty POST might return 200 (synced: 0) or 400 (Invalid JSON)
        # but here we just check it's not 401
        assert response.status_code != HTTPStatus.UNAUTHORIZED
